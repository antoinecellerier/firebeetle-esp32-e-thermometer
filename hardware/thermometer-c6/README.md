...
| GPIO | Function | Notes |
|---|---|---|
| 0 / 1 | XTAL_32K_P / N | FC-135 32.768kHz + 20pF; R9 10M bias DNP (populate only if oscillator won't start) |
...
`generator/circuit.py` — edit circuit.py, never the .kicad_sch.

## Open items (user sign-off)

1. **BMP581 sourcing** — out of stock at LCSC/JLCPCB (3 pcs). Consign from
   DigiKey/Mouser, hand-place, or wait for restock. No approved alternate
   (BMP585 is stocked but a different LGA-9 3.3×3.3 footprint).
