...
int main(void)
{
    ...
    uint8_t osr_cmd[2] = {BMP58X_REG_OSR_CONFIG, 0x00};
    ret = lp_core_i2c_master_write_to_device(LP_I2C_NUM_0, BMP58X_I2C_ADDR,
                                              osr_cmd, 2, LP_I2C_TIMEOUT_CYCLES);
    if (ret != ESP_OK) {
        wake_reason = 2;
    ...
    uint8_t odr_cmd[2] = {BMP58X_REG_ODR_CONFIG, 0x01};
    ret = lp_core_i2c_master_write_to_device(LP_I2C_NUM_0, BMP58X_I2C_ADDR,
    ...
    // 3. Wait ~2ms for conversion (1x OSR, ~1.6ms typical)
    ulp_lp_core_delay_us(3000);

    // 4. Read 3 temperature bytes (XLSB, LSB, MSB are contiguous from 0x1D)
    ...
    // 5. Store raw temperature
    temp_raw_0 = data[0];
    temp_raw_1 = data[1];
    temp_raw_2 = data[2];
    sample_count++;

    // 6. Delta comparison on LSB byte (~0.004°C resolution per count)
